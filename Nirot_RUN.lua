...
function user_setup()
	...
	state.NormalWeapon = M('Epeolatry','Lycurgos')
	state.WeaponLock = M(true, 'WeaponLocked')
	state.SIRDMode = M('SIRD','MaxEnmity')
	state.CursnaGear = M(false, 'CursnaGear')
	state.TankMode = M('Parry','DT')

	proSpells = S{'Protect','Protect II','Protect III','Protect IV','Protect V'}
	enhancingSpells = S{'Protect','Protect II','Protect III','Protect IV','Protect V','Shell','Shell II','Shell III','Shell IV','Shell V','Crusade'} --add phalanx if you want more duration at the cost of less effectiveness
	...
	send_command('bind numpad. gs c cycle TankMode')
	...
	send_command('bind numpad7 gs c cycle NormalWeapon')
...
function job_midcast(spell, action, spellMap, eventArgs)
	...
	if PhalanxAbility:contains(spell.english) then
		if UseSIRDWithPhalanx and state.SIRDMode.value == 'SIRD' then
			phalanxSet = set_combine(sets.Phalanx,sets.PhalanxSIRD)
			equip(phalanxSet)
			windower.send_command('wait 2.5;gs c update user')
		else
			do_equip('sets.Phalanx')
...
function customize_melee_set(meleeSet)
	...
	elseif state.MaxHP.value then -- most frequently going into this block
		if state.WeaponLock.value then 
			meleeSet = sets.MaxHP
		else
			meleeSet = set_combine(sets.MaxHP,{main=state.NormalWeapon.value})
		end
	else
		if state.WeaponLock.value then 
			if state.TankMode.value == 'Parry' then
				meleeSet = set_combine(meleeSet,sets.engaged.Parry)
			...
			if state.TankMode.value == 'Parry' then
				meleeSet = set_combine(sets.engaged.Parry,{main=state.NormalWeapon.value})
			else
...
function customize_idle_set(idleSet)
	...
	else  -- most frequently going into this block
		if state.MaxHP.value then
			if state.WeaponLock.value then 
				idleSet = sets.MaxHP
			else
				idleSet = set_combine(sets.MaxHP,{main=state.NormalWeapon.value})
			end
		else
			if state.WeaponLock.value then 
				-- do nothing
